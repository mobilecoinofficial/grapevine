...
//! for the API the bomb server uses to talk to the bomb enclave
...
/// Represents a serialized request for the BOMB enclave to handle
#[derive(Serialize, Deserialize)]
pub enum BombEnclaveRequest {
    /// The enclave eid, and the AKE responder id
    Init(BombEnclaveInitParams),
    ...
    /// An encrypted bomb_types::QueryRequest
    /// Respond with bomb_types::QueryResponse
...
/// The parameters needed to initialize the BombEnclave
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct BombEnclaveInitParams {
...
/// The API for the BOMB enclave
pub trait BombEnclaveApi: ReportableEnclave {
...
/// of the enclave.
pub trait BombEnclaveProxy: BombEnclaveApi + Clone + Send + Sync + 'static {}

impl<T> BombEnclaveProxy for T where T: BombEnclaveApi + Clone + Send + Sync + 'static {}
...
/// An error returned by the BOMB enclave
#[derive(Serialize, Deserialize, Debug, Display, Clone)]
pub enum Error {
    /// Sgx error: {0}
    Sgx(SgxError),
