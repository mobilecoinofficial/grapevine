...
use alloc::vec::Vec;
use core::slice;
use mc_bomb_enclave_api::{BombEnclaveApi, BombEnclaveRequest};
use mc_bomb_enclave_impl::BombEnclave;
...
#[no_mangle]
pub unsafe extern "C" fn bombenclave_call(
    inbuf: *const u8,
    inbuf_len: usize,
    outbuf: *mut u8,
    outbuf_len: usize,
...
lazy_static::lazy_static! {
    static ref ENCLAVE: BombEnclave<OcallORAMStorageCreator> = BombEnclave::new(default_logger());
}

pub fn ecall_dispatcher(inbuf: &[u8]) -> Result<Vec<u8>, sgx_status_t> {
    // Figure out what we're trying to do
    let call_details: BombEnclaveRequest = deserialize(inbuf).map_err(|err| {
    ...
    // And actually do it
    match call_details {
        BombEnclaveRequest::Init(params) => {
            mc_sgx_enclave_id::set_enclave_id(params.eid);
            serialize(&ENCLAVE.init(params))
        }
        BombEnclaveRequest::GetIdentity => mc_util_serial::serialize(&ENCLAVE.get_identity()),
        BombEnclaveRequest::NewEReport(target_info) => serialize(&ENCLAVE.new_ereport(target_info)),
        BombEnclaveRequest::VerifyQuote(quote, report) => {
            serialize(&ENCLAVE.verify_quote(quote, report))
        }
        BombEnclaveRequest::VerifyIasReport(verification_report) => {
            serialize(&ENCLAVE.verify_ias_report(verification_report))
        }
        BombEnclaveRequest::GetIasReport => serialize(&ENCLAVE.get_ias_report()),
        BombEnclaveRequest::ClientAccept(msg) => serialize(&ENCLAVE.client_accept(msg)),
        BombEnclaveRequest::ClientClose(session) => serialize(&ENCLAVE.client_close(session)),
        BombEnclaveRequest::Query(req) => serialize(&ENCLAVE.query(req)),
        BombEnclaveRequest::SetCurrentTimestamp(timestamp) => {
            serialize(&ENCLAVE.set_current_timestamp(timestamp))
        }
        BombEnclaveRequest::SetMessageTimeToLive(msg_ttl) => {
            serialize(&ENCLAVE.set_message_time_to_live(msg_ttl))
        }
    }
